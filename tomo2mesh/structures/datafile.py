...
class DataFile():
    ...
    def __init__(self, fname, data_tag = None, tiff = False,\
                 ...
                 d_shape = None, d_type = None, VERBOSITY = 1, autodetect_format = False):
        self.fname = fname
        self.data_tag = data_tag # for hdf5 only
        self.VERBOSITY = VERBOSITY
        
        # auto-detect this below
        if autodetect_format:
            self.get_format()
        else:
            self.tiff_mode = tiff
    ...
    def get_format(self):
        ...
        if not os.path.exists(self.fname):
            raise ValueError("Trying to detect file format but path does not exist:\n%s"%self.fname)
        
        # Understand input data format
        if os.path.isdir(self.fname):
            self.tiff_mode = True
        elif os.path.splitext(self.fname)[-1] in ("hdf5", "h5"):
            self.tiff_mode = False
